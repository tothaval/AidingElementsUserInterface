 ...
 * 
 *  to do
 *		add XML version and encoding on top of every XML file within the project.
 *
 *      LevelShift
 *      LevelData
 *      
 ...
 *      
 *      
 *      rework project from scratch? if so, try to refactor the needed logic into fewer lines of code,
 *      fewer classes and so on, and to distribute code logic better throughout the project. 
 *      improve on save and load order, functionality and codebase, right now it is double.
 *      rework screen and level system concept, is a SYSTEM panel needed in the way it is presented?
 *      or should i designate the zero screen in CoreCanvasSwitch or a similar class as SYSTEM, like
 *      with levelsystem?
 *      another question is about dynamic levels or screens, up to a hardcoded max amount. this way, fewer 
 *      ram would be needed if lesser levels and screens are generated by the user. 
 *      this would require a way to delete a screen. *
 *      .
 *      in case of project restart, try to further optimize element container logic
 */
...
namespace AidingElementsUserInterface
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void build()
        {
            ...
            //if (mainWindowData.screen_id == 0)  // geht nicht, weil SYSTEM_CANVAS anders kommuniziert wird
            // screen_id 0 ist userspace, müsste per bool hinterlegen, ob SYSTEM_CANVAS active war und daraus 
            // lesen und ableiten, was zu tun ist.
            //{
            //    set_SYSTEM_CANVAS();
            //}
            //else
            //{
            //    CORE_CANVAS_SWITCH.jumpToScreen(mainWindowData.screen_id);
            //}
        ...
        public void quitAEUI()
        {
            ...
            mainWindowData.screen_id = CORE_CANVAS_SWITCH.Get_ACTIVE_CANVAS_ID();
        ...
        private void MI_SessionClock_Click(object sender, RoutedEventArgs e)
        {
            summonElement(sender);
        }
