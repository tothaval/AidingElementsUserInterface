...
namespace AidingElementsUserInterface.Elements.FlatShareCC
{
    ...
    public partial class AnnualBillingUI : UserControl
    {
        ...
        internal CostUpdateData extract_data()
        {
            ...
            foreach (CoreDataInputElement die in SP_room_consumption.Children)
            {
        ...
        private void setup_DIEs()
        {
            ...
            // double value DIEs
            DIE_initial_cold_rent.configurate("cold rent", false, "0", Units.currency);
            DIE_initial_extra_costs_advance.configurate("extra costs advance", false, "0", Units.currency);
            DIE_annual_costs.configurate("annual costs", false, "0", Units.currency);
            DIE_extra_costs_shared.configurate("extra costs shared", false, "0", Units.currency);
            DIE_extra_costs_heating.configurate("extra costs heating", false, "0", Units.currency);

            DIE_heating_units_usage.configurate("heating units consumption", false, "0", Units.heating);
            DIE_heating_units_shared.configurate("shared consumption", false, "0", Units.heating);
        ...
        private void setup_room_DIEs()
        {
            ...
            {
                CoreDataInputElement DIE_room = new CoreDataInputElement();

                DIE_room.id = room.id;
                DIE_room.configurate($"{room.name} - {room.area}{Units.area}", false, Units.heating);
        ...
        private void DIE_room_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter || e.Key == Key.Return)
            {
                CoreDataInputElement room_die = (CoreDataInputElement)sender;
