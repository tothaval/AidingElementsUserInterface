...
{
    /// <summary>
    /// Interaktionslogik für CostOutputUI.xaml
    /// </summary>
    public partial class CostOutputUI : UserControl
    {
        ...
        internal CostOutputUI(int room_id, CostUpdateData data)
        {
            InitializeComponent();
        ...
        private void _room_name()
        {
            if (cud != null)
            {
                LL_room_name.Content = $"{cud.roomConsumptionValues[rnr - 1].room.name} - {cud.roomConsumptionValues[rnr - 1].room.area}{Units.area}";
            }
        ...
        private void calculate_cost_update()
        {
            ...
            FlatShareCC_Calculations calculate = new FlatShareCC_Calculations(cud);
        ...
        {
            DIE_room_area.configurate("room", true, Units.area);
            DIE_shared_area.configurate("share", true, Units.area);
            DIE_total_area.configurate("total", true, Units.area);

            DIE_payments_cr.configurate("rent", true, Units.currency);
            DIE_payments_ec.configurate("costs", true, Units.currency);
            DIE_payments.configurate(">", true, Units.currency);

            DIE_cold_rent_costs.configurate("rent", true, Units.currency);
            DIE_extra_costs_share.configurate("house", true, Units.currency);
            DIE_extra_costs_heating.configurate("heating", true, Units.currency);

            DIE_balance_total.configurate("=", true, Units.currency);
            DIE_costs_total.configurate("-", true, Units.currency);

            DIE_cold_rent_diff.configurate("rent", true, Units.currency);
            DIE_ecs_diff.configurate("house", true, Units.currency);
            DIE_ech_diff.configurate("heating", true, Units.currency);
